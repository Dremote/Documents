  ...
  )
{
  ...
  DirEnt = NULL;
  Volume = OFile->Volume;
  DEBUG((-1,"FatOFileOpen Enter\n"));
  DEBUG((-1,"FatOFileOpen OpenMode is %x\n",OpenMode));
  ...
  DEBUG((-1,"FatLocateOFile Exit\n"));
  if (EFI_ERROR (Status)) {
      DEBUG((-1,"FatLocateOFile:%r",Status));
    return Status;
  }
